...
class Maps():
    ...
    def rescale_maps(self, floors, map_floors, aps, old_esx_path, new_esx_path):
        ...
        if len(update_maps) == 0:
            self.map_match = False
            print(f"{self.status.no}No matching floorplans{colorama.Fore.RESET}")
        ...
        for ap in aps:
            if ap.location_name in update_maps.keys():
                if ap.location_x != "" and ap.location_y != "":
                    ...
                    old_x_offset_ratio = (ap.location_x - old_x1) / (old_x2 - old_x1)
                    new_x_offset_value = (new_x2 - new_x1) * old_x_offset_ratio
                    new_ap_x = new_x1 + new_x_offset_value

                    old_y_offset_ratio = (ap.location_y - old_y1) / (old_y2 - old_y1)
                    new_y_offset_value = (new_y2 - new_y1) * old_y_offset_ratio
                    new_ap_y = new_y1 + new_y_offset_value
        ...
        print(f"{self.status.ok}Replaced images: {colorama.Fore.GREEN}{len(update_maps)}{colorama.Fore.RESET}")
        print(f"{self.status.ok}Repositioned APs: {colorama.Fore.GREEN}{len(update_aps)}{colorama.Fore.RESET}")
